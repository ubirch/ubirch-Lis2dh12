...
int16_t Lis2dh12::init() {
  ...
  if (whoamI != LIS2DH12_ID) {
    EDEBUG_PRINTF(
        "Sensor ID check failed! (Expected ID: 0x%02x - Got: 0x%02x)\r\n",
        LIS2DH12_ID, whoamI);
    return 0xffff;
  } else {
...
int16_t Lis2dh12::enableSensor() {
  ...
  return writeReg(LIS2DH12_CTRL_REG1, (uint8_t *)&ctrlReg1,
                  1); // turn on sensor
}
...
int16_t Lis2dh12::setOperatingMode(lis2dh12_odr_t _sampRate,
...
int16_t Lis2dh12::initHPF(uint8_t hpcf) {
  EDEBUG_PRINTF("High-pass filter cutoff frequency: %x\r\n\r\n", (hpcf & 0b11));
  ...
  for (int i = 0; i < ACC_ARRAYSIZE; i++) {
    ...
    if (debug)
      EDEBUG_PRINTF("%02d: %5d | %5d | %5d \r\n", i,
                    accelerationArray[i].x_axis, accelerationArray[i].y_axis,
...
int16_t Lis2dh12::disableDoubleClickInterrupt() {
  int16_t error = 0;
...
int16_t Lis2dh12::convert_to_mg(int16_t rawData) {
  ...
  default:
    EDEBUG_PRINTF(
        "ERROR converting raw acceleration data: undefined full-scale\r\n");
    return 0xffff;
  }
}
